...
const Chat = () => {
  ...
  const id =
    ...
    conversation?.data?.[0]?.participants?.[0]?._id;
  ...
  return (
    <div className="container mx-auto lg:grid grid-cols-7 mt-8 gap-6 ">
      <div
        className={` ${
          tab ? "col-span-7 lg:col-span-3" : "col-span-3 hidden lg:block"
        } bg-white p-3 rounded-md`}
      >
        <Input className=" py-2 " prefix={<IoSearch />} />
        <p className="text-lg text-[#4E4E4E] font-semibold my-2 ">Chats</p>
        <div
          className={`flex flex-col gap-4 justify-start items-start w-full min-h-[50vh]`}
        >
                ...
                onClick={() => {
                  ...
                  window.history.pushState(
                    ...
                    `${window.location.pathname}?${(() => {
                      ...
                      params.set("id", item?.participants?.[0]?._id);
                      return params.toString();
                    })()}`
                  );
                }}
                className="flex justify-between items-start gap-2 hover:bg-black hover:bg-opacity-10 w-full rounded-md p-1 cursor-pointer"
                key={item?._id}
              >
                <div className="flex justify-start items-center gap-2">
                  <img
                    className="w-16 h-16 rounded-full object-cover"
                    src={imageUrl(item?.participants?.[0]?.profile_image)}
                    alt=""
                  />
                  <div className="">
                    <p className="text-[#4E4E4E] text-lg font-medium">
                ...
                <p className="text-end text-sm text-[#6B6B6B]">
                  {moment(
      ...
      <div
        className={`${
          tab ? " col-span-4 hidden lg:block" : "col-span-7 lg:col-span-4"
        } bg-white p-3 rounded-md`}
      >
        {data?.userDetails?.profile_image ? (
          <div className="flex justify-between items-center gap-2  w-full rounded-md p-1 pb-4 border-b">
            ...
            >
              ...
              <img
                className="w-16 h-16 rounded-full object-cover"
                src={imageUrl(data?.userDetails?.profile_image)}
                alt=""
              />
              <div>
                <p className="text-[#4E4E4E] text-lg font-medium">
            ...
            messages?.map((item, i) => {
              return (
                <>
                  <div
                    key={item?._id}
                    className={`w-fit max-w-[70%] text-white px-6 rounded-md py-2 my-2 relative ${
                      item?.receiverId !== id
                        ? "mr-auto ml-4 bg-[#6B6B6B]"
                        : "ml-auto mr-4 bg-[#1A66FF]"
                    }`}
                  >
                    <p>{item?.message}</p>
                    <BsFillTriangleFill
                      className={`text-2xl ${
                        item?.receiverId !== id
                          ? "text-[#6B6B6B] -left-2 bottom-[-1px]"
                          : " text-[#1A66FF] -right-2  bottom-[-1px]"
                      } absolute `}
                    />
                  </div>
                  {item?.message_img && (
