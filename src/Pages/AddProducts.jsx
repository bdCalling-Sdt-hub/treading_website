                ...
                <div className='md:grid flex flex-col justify-start md:justify-center items-start md:items-center grid-cols-2 gap-4 md:gap-6'>
                    ...
                    >
                        <Select onChange={(value) => {
                            const filterCategory = category.filter(item => item?._id === value)
                            setSelectedCategory(filterCategory[0] || {})
                        }} className='min-w-56' placeholder='choose a category' options={category?.map(item => ({ label: item?.name, value: item?._id })) || []} />
                    </Form.Item>
                    <Form.Item
                        name={`subCategory`}
                        className='w-full'
                    ...
                    >
                        <Select className='min-w-56' placeholder='choose a sub category' options={selectedCategory?.subcategories?.map(item => ({ label: item?.name, value: item?._id })) || []} />
                    </Form.Item>
                    <Form.Item
                        className='w-full'
                        name={`title`}
